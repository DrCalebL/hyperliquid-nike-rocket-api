...
class OpenPosition(Base):
    ...
    # Position details
    symbol = Column(String, nullable=False)  # BTC/USDT format (API format)
    hl_coin = Column(String, nullable=True)  # Hyperliquid coin e.g. "BTC"
    ...
    # Status: open, closed, closed_manual, needs_review, error
...
class PositionFill(Base):
    ...
    # Timing
    fill_timestamp = Column(DateTime, nullable=True)
    recorded_at = Column(DateTime, default=datetime.utcnow)
    
    # Source
    source = Column(String, default='hl_api')


class PortfolioSnapshot(Base):
    """Balance snapshots for deposit/withdrawal detection"""
...
class PortfolioTransaction(Base):
    """Detected deposits and withdrawals"""
...
class Trade(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    user_id = Column(Integer, ForeignKey("follower_users.id"), nullable=False)
    signal_id = Column(String, nullable=True)  # String to match position monitor
    ...
    # Exit type (TP, SL, MANUAL, UNKNOWN)
    exit_type = Column(String, nullable=True)
    
    # Fee tracking (30-day billing: always 0 per-trade)
...
def init_db(engine):
    ...
    required_tables = [
        ...
        'balance_snapshots', 'error_logs', 'position_fills',
...
__all__ = [
    'Base', 'User', 'Signal', 'SignalDelivery', 'OpenPosition', 'PositionFill',
    'Trade', 'Payment', 'SystemStats', 'BalanceSnapshot', 'ErrorLog',
    'PortfolioSnapshot', 'PortfolioTransaction',
